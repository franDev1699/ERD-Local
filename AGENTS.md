...
- `src/core/`: Core logic and state management.
    - `StateManager.js`: Central source of truth for the application state. Manages `appState` and notifies subscribers of changes.
...
- `src/services/`: Infrastructure and external integrations.
    ...
    - `ExportService.js`: Logic for exporting data (JSON, SQL, Images).
- `src/ui/`: User Interface components.
    ...
    - `SidebarEditor.js`: Manages the side panel for table and field editing.
    - `UIManager.js`: Manages global UI elements (Toasts, Modals, Search).
- `src/controllers/`: Orchestration and event handling.
    - `AppController.js`: The main orchestrator that initializes services and connects UI to state.
    - `InteractionController.js`: Handles low-level user interactions (mouse dragging, connection drawing, keyboard events).
